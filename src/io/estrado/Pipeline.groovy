...
def helmDeploy(Map args) {
    ...
    if (args.dry_run) {
        ...
        sh "helm upgrade --dry-run " + (args.debug ? "--debug " : " ") + "--install ${args.name} ${args.chart_dir} " + (release_overrides ? "--set ${release_overrides} " : " ") + (args.timeout ? "--timeout ${args.timeout} " : " ") + " --namespace ${namespace}"
    } else {
